...
#ifdef _WIN32
    return ::send(socket_, data.c_str(), data.size(), 0);
#else
    size_t bytes_written;
    OSStatus status = SSLWrite(context_, data.c_str(), data.size(), &bytes_written);
    if (status == noErr) {
...
std::string ssl_socket::receive() {
    ...
    do {
        ...
        std::clog << "SSLRead status: " << status << ", bytes_received: " << bytes_received << std::endl;
    ...
    if (status == noErr || status == errSSLWouldBlock) {
        ...
        // Handle error
        std::cerr << "SSLRead error: " << status << std::endl;
        return "";
    }
#endif
}
...
bool ssl_socket::perform_handshake() {
    ...
    if (AcquireCredentialsHandle(NULL, UNISP_NAME, SECPKG_CRED_OUTBOUND, NULL, &scCred, NULL, NULL, &hCred_, NULL) != SEC_E_OK) {
        return false;
    }
    ...
    pbIoBuffer = (PBYTE)LocalAlloc(LMEM_FIXED, 0x10000);
    if (!pbIoBuffer) {
        return false;
    }
    ...
    SSLSetIOFuncs(context_, [](SSLConnectionRef connection, void *data, size_t *dataLength) -> OSStatus {
        int socket = *reinterpret_cast<int*>(const_cast<void*>(connection));
    ...
    SSLSetConnection(context_, reinterpret_cast<const void*>(&socket_));
    SSLSetPeerDomainName(context_, host_.c_str(), host_.length());
